...
// Библиотека Уфа-Скрипт версия 2.0 от 16.04.2017 предназначена для OneScript 
...
Функция ВерсияUfascript(Вид)  экспорт
   Версия = "2.0"; ДатаВерсии="16.04.2016";
...
Функция СоздатьООПОбъект(ИмяОбъекта,М)   экспорт
   ...
   С=ЗагрузитьСценарий(А);   С.ИмяОбъекта = ИмяОбъекта;  С.НашОбъект = С;
...
Процедура ОбновитьЭкранОбъектов()  экспорт
   Для сч=0 По Экран.Количество()-1 Цикл 
     Если НЕ Экран[сч]=0 Тогда 
	    Экран[сч].Нарисовать();
	 КонецЕсли;  
	КонецЦикла;
КонецПроцедуры      //     Экран[сч].Нарисовать();

#КонецОбласти

#Область ОбразыОбъектовООП

Функция ОбразОбъектБазовый(М) экспорт    
   М.Добавить("Перем Экран Экспорт; Перем Индекс Экспорт;  Перем ТипОбъекта Экспорт; ");
...
Функция СоздатьСтрокуТекста(Y1,X1,Текст1,ЦФ,ЦТ,Имя,U) экспорт
  ...
  П1.РегистрацияНаЭкране(ЭтотОбъект,U);  П1.Нарисовать();  
  Возврат П1;
КонецФункции

Функция ОбразОбъектЭкрана(М) экспорт      //  ТипОбъекта = 1   ------------------
   ...
   М.Добавить("Перем Y экспорт; Перем X экспорт; Перем Текст Экспорт; "+
    ...
    М.Добавить("Функция Конструктор() экспорт Y=1; X=1; ТипОбъекта=1; Текст=ИмяОбъекта; ЦветФона = ЦветКонсоли.Зеленый; "+
       "ЦветТекста = ЦветКонсоли.Черный;  КонецФункции ");
...
Функция СоздатьПолеВвода(Y1,X1,Текст1,Имя,U) экспорт
  П1=СоздатьООПОбъектИзМассива(Имя,ОбразОбъектПолеВвода(Новый Массив));
  П1.Y = Y1;    П1.X = X1;  П1.Текст = Текст1;
  П1.РегистрацияНаЭкране(ЭтотОбъект,U);  П1.Нарисовать();  
  Возврат П1;
КонецФункции

Функция ОбразОбъектПолеВвода(М) экспорт    //  ТипОбъекта = 10   ------------------------
   ...
   М.Добавить("#Переименовать Конструктор РодитКонструктор");
    М.Добавить("Функция Конструктор() экспорт   РодитКонструктор(); Y=10; X=10; ТипОбъекта=2;  ДлинаТекста = 25; "+
     "	ЦветФона = ЦветКонсоли.Белый; ЦветТекста = ЦветКонсоли.Черный;   КонецФункции ");
    М.Добавить("Функция Действие() экспорт UfaScript.Консоль.ЦветФона = ЦветКонсоли.Белый;  UfaScript.Консоль.КурсорЛево = X; "+
         " ВвестиСтроку(Текст,30);   Возврат Истина; "+
                      "     КонецФункции ");  //  
	   Возврат М;
КонецФункции                       ///  Текст = UfaScript.ПереходВРедактированиеСтроки("""",ДлинаТекста); 

Функция СоздатьПрямоугольник(Y,X,Имя,U,МТ) экспорт
  Кн1=СоздатьООПОбъектИзМассива(Имя,ОбразОбъектКнопка(Новый Массив));  Кн1.ТипОбъекта = 2;
  Кн1.РегистрацияДанных(Y,X," "," ",ЭтотОбъект,U); Кн1.МассивТекста=МТ; Кн1.Нарисовать();  
  Возврат Кн1;
КонецФункции

Функция ОбразОбъектПрямоугольник(М) экспорт     //  ТипОбъекта = 2   ------------------------
   ...
   М.Добавить("Функция Нарисовать() экспорт  "+
     " UfaScript.ВысветитьОбъектВЦвете(МассивТекста, Y, X, ЦветФона, ЦветТекста);  КонецФункции ");
   М.Добавить("Функция РегистрацияДанных(Y1,X1,Текст1,ФД,U,Гл) экспорт  Y=Y1; X=X1; Текст=Текст1;"+
	" Изобразить();   ТипОбъекта=2; РегистрацияНаЭкране(U,Гл); Функц=ФД; КонецФункции ");
   М.Добавить("Функция Изобразить() экспорт  МассивТекста=0;   КонецФункции ");
   Возврат М;
КонецФункции
...
Процедура НарисоватьООПРамкуНаВесьЭкран(U) экспорт
    ...
    Кн1.РегистрацияДанных(0,0," "," ",ЭтотОбъект,U); Кн1.МассивТекста=М; 
	Кн1.ЦветФона=ЦветКонсоли.Желтый;  Кн1.ЦветТекста=ЦветКонсоли.Красный;  Кн1.Нарисовать();  
КонецПроцедуры

Функция СоздатьКнопку(Y,X,Имя,ФД,U,МТ) экспорт
  Кн1=СоздатьООПОбъектИзМассива(Имя,ОбразОбъектКнопка(Новый Массив));   Кн1.ТипОбъекта = 11;
  Кн1.РегистрацияДанных(Y,X," ",ФД,ЭтотОбъект,U); Кн1.МассивТекста=МТ; Кн1.Нарисовать();  
  Возврат Кн1;
КонецФункции

Функция ОбразОбъектКнопка(М) экспорт
   М=ОбразОбъектПрямоугольник(М);   М.Добавить("Перем Функц;");      
   М.Добавить("Функция Действие() экспорт   UfaScript.ВызватьФункцию(Функц,ГлМодуль).Результат();  КонецФункции ");    
   Возврат М;
КонецФункции

Функция СоздатьКнопку20(Y,X,Текст,Имя,ФД,U) экспорт
  Кн1=СоздатьООПОбъектИзМассива(Имя,ОбразОбъектКнопка20(Новый Массив));
  Кн1.СоздатьМассивКнопки(Y,X,Текст,ФД,ЭтотОбъект,U);   Кн1.Нарисовать();  
  Возврат Кн1;
КонецФункции

Функция ОбразОбъектКнопка20(М) экспорт        //  ТипОбъекта = 11   ------------------------
  ...
  "       М1=Новый Массив; М1.Добавить(""+-----------------+""); "+
                      "    М1.Добавить(""|"" + Текст +   ""|""); "+
                      "    М1.Добавить(""+-----------------+""); МассивТекста=М1; КонецФункции ");
   Возврат М;
КонецФункции

Функция ОбразОбъектКнопкаМассив(М) экспорт
   М=ОбразОбъектКнопка(М);
   М.Добавить("#Удалить Изобразить");   
   М.Добавить("Функция Изобразить() экспорт  МассивТекста=0;   КонецФункции ");
   Возврат М;
...
Функция ОбразОбъектГруппы(М) экспорт
   М=ОбразОбъектБазовый(М);
   М.Добавить("Перем МассивГруппы Экспорт; Перем АктивИндекс экспорт;  Перем АктивЦветФона  экспорт; "+
       "Перем ЦветФона  экспорт; ");
   М.Добавить("Функция Конструктор() экспорт МассивГруппы = Новый Массив; АктивИндекс=-1; "+
       "АктивЦветФона=ЦветКонсоли.Синий; ТипОбъекта=20;  КонецФункции ");
   М.Добавить("Функция ДобавитьЭлементВГруппу(Об) экспорт UfaScript.Экран[Об.Индекс]=0; АктивИндекс=АктивИндекс+1; "+
       "Об.Индекс= МассивГруппы.Количество();   МассивГруппы.Добавить(Об); Об.Экран=НашОбъект; ЦветФона=Об.ЦветФона; КонецФункции ");
   М.Добавить("Функция АктивироватьЭлемент(Инд) экспорт  МассивГруппы[АктивИндекс].ЦветФона=ЦветФона; "+
     " Если Инд<0 Тогда Возврат 0; КонецЕсли; "+
     " МассивГруппы[АктивИндекс].Нарисовать(); АктивИндекс=Инд; "+
       "  МассивГруппы[Инд].ЦветФона=АктивЦветФона;  МассивГруппы[Инд].Нарисовать();  КонецФункции ");
   М.Добавить("Функция Нарисовать() экспорт   Если МассивГруппы.Количество()>0  Тогда  "+
       " Для сч=0 По МассивГруппы.Количество()-1 Цикл  Если МассивГруппы[сч]<>0 Тогда  "+
       "  МассивГруппы[сч].Нарисовать();   КонецЕсли; КонецЦикла; КонецЕсли;  КонецФункции ");
   М.Добавить("Функция ОбработатьНажатия(Клавиша) экспорт Если Клавиша=27 Тогда  Возврат Ложь; ИначеЕсли "+
       "  Клавиша=38 Тогда  Если АктивИндекс>0 Тогда АктивироватьЭлемент(АктивИндекс-1); КонецЕсли;    ИначеЕсли "+
        "  Клавиша=40 Тогда Если АктивИндекс<МассивГруппы.Количество()-1  Тогда АктивироватьЭлемент(АктивИндекс+1); "+
		  " КонецЕсли; ИначеЕсли Клавиша=13 Тогда  МассивГруппы[АктивИндекс].Действие(); КонецЕсли;   Возврат Истина; КонецФункции "); 
    Возврат М;
КонецФункции
...
Функция ОбразОбъектГруппыГоризонт(М) экспорт
   ...
   М.Добавить("Функция ОбработатьНажатия(Клавиша) экспорт Если Клавиша=27 Тогда  Возврат Ложь; ИначеЕсли "+
       "  Клавиша=37 Тогда  Если АктивИндекс>0 Тогда АктивироватьЭлемент(АктивИндекс-1); КонецЕсли;    ИначеЕсли "+
        "  Клавиша=39 Тогда Если АктивИндекс<МассивГруппы.Количество()-1  Тогда АктивироватьЭлемент(АктивИндекс+1); "+
		  " КонецЕсли; ИначеЕсли Клавиша=13 Тогда  МассивГруппы[АктивИндекс].Действие(); КонецЕсли;   Возврат Истина; КонецФункции "); 
    Возврат М;
КонецФункции
#КонецОбласти
...
Функция ЗагрузитьСтрокиИзФайлаВМассив(Имя)  экспорт
  М = Новый Массив;  Чтение = Новый ЧтениеТекста(Имя);	  Стр = Чтение.ПрочитатьСтроку();	
  Пока Стр <> Неопределено  Цикл     М.Добавить(Стр);     Стр = Чтение.ПрочитатьСтроку();	  КонецЦикла;	  
