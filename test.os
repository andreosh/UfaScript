...
U.НарисоватьРамкуНаВесьЭкран();    U.Консоль.Прочитать();
 ...
 //  Ф2=U.СоздатьНаследникаООПОбъекта("ОбъектЭкрана","Собачка",ОбразОбъектКотик(Новый Массив));  
   ...
   Ф2.РегистрацияНаЭкране(U);   Ф2.Y = 23;   Ф2.Нарисовать();  

  Кн1=U.СоздатьООПОбъектИзМассива("Кнопка",ОбразОбъектКнопка(ОбразОбъектЭкрана(Новый Массив)));
  Кн1.СоздатьМассивКнопки(40,40,"     Жми !       ",U);    Кн1.Нарисовать();  
...
Функция ОбразОбъектКнопка(М)
  М.Добавить("Перем МассивТекста;");     М.Добавить("#Удалить Нарисовать");   
  М.Добавить("Функция Нарисовать() экспорт  "+
     " UfaScript.ВысветитьОбъектВЦвете(МассивТекста, Y, X, ЦветФона, ЦветТекста);  КонецФункции ");
  М.Добавить("Функция СоздатьМассивКнопки(Y1,X1,Текст1,U) экспорт  Y=Y1; X=X1; Текст=Текст1;"+
	" Изобразить();   РегистрацияНаЭкране(U);    КонецФункции ");
...
Функция ОбразОбъектЭкрана(М)
   ...
   "Перем ЦветФона Экспорт; Перем ЦветТекста Экспорт; Перем Индекс Экспорт;  Перем Направление экспорт;");
   М.Добавить("Функция Конструктор() экспорт Y=1; X=1; Текст=ИмяОбъекта; ЦветФона = ЦветКонсоли.Зеленый; "+
       "ЦветТекста = ЦветКонсоли.Черный; Направление=0;  КонецФункции ");
...
Функция ВызватьФункцию(Функ,T)
  М = Новый Массив;    М.Добавить("Т=ЗагрузитьСценарий(""UfaScript.osb"");");
  М.Добавить("Возврат Т."+Функ+";");  Возврат U.ВыполнитьКодИзМассива(М);
КонецФункции
